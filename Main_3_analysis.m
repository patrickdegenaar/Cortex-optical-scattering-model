...
%
%  NOTE: this is only really necessary for smaller photon numbers <1e7 in
%  the Monte Carlo simulation. My Mac laptop does takes 1.5 hours to do 1e8 
%  whereas my old desktop would take almost 36 hours. It is better not to
%  use this for accuracy - but it does help smooth out discontinuities for
%  faster simulations
%--------------------------------------------------------------------------
% gaussianSize = 11;
% GaussianDist = 0.75;
...
% I'm not using the smoothing process but you might need to:
smoothedImage_xyz = scatterImage_xyz;
...
%--------------------------------------------------------------------------
sumVals = zeros(1, simBoxSizeZ);
for i = 1:simBoxSizeZ
    sumVals(i) = sum(sum(smoothedImage_xyz(:,:, i)));
end
[~, idx] = max(sumVals);
...
maxVal = max(max(max(log10(calibratedImage_xyz))));

subplot(2,2,1);
pcolor(zAxis, yAxis, log10(calibratedImage_yz));
shading interp; axis equal tight; title('YZ plane'); clim([maxVal-3 maxVal]);

subplot(2,2,2);
pcolor(zAxis, xAxis, log10(calibratedImage_xz));
shading interp; axis equal tight; title('XZ plane'); clim([maxVal-3 maxVal]);

subplot(2,2,3);
pcolor(xAxis, yAxis, log10(calibratedImage_xy));
shading interp; axis equal tight; title('XY plane'); clim([maxVal-3 maxVal]);
...
imwrite(writeIm_xy, 'GeneratedData/linearXY.jpg');
imwrite(writeIm_xz, 'GeneratedData/linearXZ.jpg');
imwrite(writeIm_yz, 'GeneratedData/linearYZ.jpg');
imwrite(writeIm_Log_xy, 'GeneratedData/logXY.jpg');
imwrite(writeIm_Log_xz, 'GeneratedData/logXZ.jpg');
imwrite(writeIm_Log_yz, 'GeneratedDatalogYZ.jpg');

%--------------------------------------------------------------------------
%% Per-layer total power (energy-conservation check)
%
%  In a purely absorbing medium, integrating over each constant-z slab
%  gives the total power crossing that depth.  
% 
%  For a purely forward scattering case, this should be a flat line.
%  However, anisotropy means a proportion (~10%) of the photons will
%  backscatter. Then when we include absorption, we will see a decay due to
%  absorption of photons.

%--------------------------------------------------------------------------

% get the central calibrated data 
Centraldata = squeeze(calibratedImage_xyz(YZplane_x, XZplane_y, :));

% get the integral layer data
layerData = zeros(1, simBoxSizeZ);
for layer = 1:simBoxSizeZ
    layerData(layer) = sum(sum(calibratedImage_xyz(:, :, layer))) * voxel_area;
end

% Plot the intensity dropoff
figure; hold on
plot((1:simBoxSizeZ)*z_div, Centraldata);
xlabel('z (\mum)'); ylabel('intensity in mW/mm2 - assuming 1mW source');
title('Energy-conservation check'); grid on;

% normalise so that they both 
Centraldata = Centraldata/max(Centraldata);
layerData = layerData/max(layerData);

figure; hold on
plot((1:simBoxSizeZ)*z_div, layerData);
plot((1:simBoxSizeZ)*z_div, Centraldata);
xlabel('z (\mum)'); ylabel('Normalised power');
title('Energy-conservation check'); grid on;
legend('Layer-integrated power', 'Central intensity'); 
